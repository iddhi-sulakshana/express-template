...
response.sendResponse = function <T>(
  ...
  if (response instanceof Error) {
    ...
    winston.error(
      `Error sending request: [${this.req.originalUrl}] : ` + response.message,
      response,
    );
  ...
  if (isDownloadResponse(response)) {
    return this.download(response.filePath, response.fileName, (err) => {
      if (err) {
        winston.error(`Download Error: [${this.req.originalUrl}] : ` + err.message, err);
        if (!this.headersSent) {
  ...
  if (isViewResponse(response)) {
    return this.sendFile(response.filePath, (err) => {
      if (err) {
        winston.error(`Send File Error: [${this.req.originalUrl}] : ` + err.message, err);
        if (!this.headersSent) {
          this.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send({
            status: HTTP_STATUS.INTERNAL_SERVER_ERROR,
            message: 'Internal Server Error',
